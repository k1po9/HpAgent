...
============================================================================
  ...
  resources/                 资源层（模型调用池 + 凭据管理）
    ...
    ↓
  sandbox/                   沙箱层（手——工具选择 + 安全执行 + 输出后处理）
    ├── sandbox.py           单个沙箱：工具选择管线 + 安全路由执行 + hints 跨轮状态
  ...
  session/                   会话层（记忆 + 工作区）
    ...
    ├── workspace.py         会话工作区目录初始化（通过 LocalFileStore）
    └── db.py                WorkspaceDB：SQLite 用户 + 会话元数据
    ↓
  storage/                   存储层（Redis 缓存 + 本地文件）
    ├── redis.py             RedisCache
    ├── file_store.py        LocalFileStore：基于 FileStore 协议的文件 I/O
  ...
  common/                    公共层
    ...
    ├── interfaces.py        核心接口 ABC（IResources / IChannel）
