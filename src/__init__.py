...
============================================================================
  ...
  sandbox/                   沙箱层（手——工具执行 + 渠道 I/O）
    ...
    ↓
  session/                   会话层（记忆——当前已抽空，待基于 storage/ 重建）
    ├── models.py            领域模型：Session / EventRecord / SessionStatus
    └── session_manager.py   TemporalSessionManager（通过 Workflow Query 读事件）
    ↓
  ...
  account/                   账号层（跨渠道用户身份统一）
    ...
    ↓
  common/                    公共层（接口 / 类型 / 错误）
    ├── interfaces.py        IResources / ISandbox / ISession / IChannel 接口协议
