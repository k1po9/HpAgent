...
| W1 唯一 durable 主线 | W0；01/04/08/17 | 冻结 source-neutral 输入、共享 Run owner、Run lifetime ≠ Execution lease lifetime；实现通用 suspend/resume、wait 释放资源、重新 acquire 与新 fencing token 传播，G03/G04 验证。Web 先验证 ReAct/Plan、终态/取消/恢复；已有 QQ 临时尚在旧链，不可称双入口目标完成 |
| W2 Conversation/QQ 收敛 | W1；01/04/07 | QQ 改调 PG 命令+Outbox，统一消息幂等/Session/PG admission authority/身份（首版推荐 busy reject），投递与执行分开；两个 surface 同一主线验证 G06。按需救出 DTO/错误等目标依赖 |
...
| W5 Model Input 边界 | W4；12/17 | 在 W1 已支持 suspend/resume 的合同上接入 Model Review；不重构 Run/lease 基础生命周期。拆 Prepare/Invoke、冻结 snapshot/provider body/AuthorizationPolicy，接入授权、失效与投影。首版 off/every_call 都走同一逐快照授权链，G12 覆盖策略校验与人工等待 |
...
## W1 suspend-safe 合同与退出条件

逻辑 Run 可以 running → waiting_review / waiting_approval / external_wait → running → completed；状态如何映射 PG 字段由 W1 冻结。Run ID 与逻辑 operation ID 跨等待保持稳定，execution lease/fencing token 仅属于当次活跃执行区间。进入 durable wait 前持久化恢复点并释放 execution lease、Activity slot 与 workspace lock，不跨等待保留数据库事务。恢复先检查 Run 未取消/终结，重新 acquire 并将新 token 传给后续模型、工具与 lease release；拒绝旧 token 的迟到写入，重验相关输入依赖。
...
| G03 目标合同 | W1 验证中立 Run 输入支持无 Conversation/Message/Session 的非 Chat 合同样例（不实现未来入口），token 不固定为整条 Run 生命周期；两 surface DTO/权限/错误/幂等、Workflow/Activity registration、操作 ID、配置与 schema 和选定目标一致；允许修改旧名称/接口，需更新消费者，不能为过旧测试保留实现 | test/test_durable_agent_contract.py、test/test_action_result_semantics.py、test/web_api/test_config.py；新增 canonical registry、无 legacy flag/导出断言 |
| G04 Durable 行为 | W1 必须通过上述 suspend-safe 场景，证明等待零长期资源占用、恢复新 fencing token 与旧 token 拒绝；ReAct/Plan/step/replan/final、完成/失败/取消、worker kill、审批恢复、lease/fencing、预算、工具 uncertain、Outbox Start 重投均达预期；目标 History deterministic replay，通过 PG/Temporal 故障场景 | test/test_durable_agent_temporal_integration.py、test/test_durable_agent_worker_kill.py、test/web_persistence/test_durable_agent_activity_worker_kill.py、test/test_tool_execution_approval_temporal.py；建立目标 replay fixture |
...
| G06 双入口领域 | 所有 surface 服从同一 PG admission authority；首版 single active + busy reject 是推荐策略，替换策略仍须该 authority 决定；PG 同一命令与 Session/Run 规则；QQ 重投只接受一次；相同身份不串 Conversation；群聊/私聊授权、busy、轮换、取消/retry、记忆来源正确；投递重试不重执行 Agent，未绑定与 DB 失败区别明确 | test/test_qq_execution_compat.py 的业务场景迁入新测试；test/web_persistence/test_phase_c_sessions.py、test/web_api/test_sse_gateway.py；必须补 QQ → PG → Outbox → durable → delivery E2E |
...
| G12 Model Review | 06 的不变量：每个 snapshot 在 Invoke 前满足冻结 AuthorizationPolicy，无旁路；off/every_call 均逐次核验，扩展模式合同允许 first_call_only/phase_based/policy-based 而不实现这些模式；所有主 Agent phase 都覆盖；approve/reject/expiry/cancel、wait 重启、重复/伪造/错 hash/跨 owner/旧 snapshot 信号、输入改变/模型 fallback 生成新快照并重新评估策略，需人工时重审；权限视图同 canonical hash；发送 body 与获授权快照一致；复用 W1/G04 基础合同验证 Model Review 集成后的恢复新 token、CAS 与预算 | 当前 model_decision/planning/evaluate/ModelClient 仅为重构入口；文件 approval 测试可借场景但不是已覆盖。必须新增 snapshot/payload 与模型假服务断言 |
