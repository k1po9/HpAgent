# Canonical Architecture · 目标边界 R2.1
...
flowchart TD
    ...
    RT --> D
    S[Future Scheduled / Automation / File trigger] -.-> SR[Source-owned context + shared Run / Outbox]
    SR -.-> D
    D -->|Agent Run| L[Source-neutral Agent lifecycle]
    ...
    L --> F[PG terminal state + source events]
    F --> WS[Web SSE adapter]
    F --> QS[QQ delivery adapter]
```

图中中立 Agent lifecycle 由现有 durable 生命周期演进，不新增并存实现，也不以 Conversation 命名冻结永久依赖。未来来源虚线仅是合同扩展口，尚未实现；Chat source 将结果事件映射为 delivery，非 Chat 不强造 Message/Session 或聊天投递。Dispatcher 可按 run_kind 选择入口；业务 Workflow 内不能变成 agent/research/artifact/document 分支大全。
...
| Run | 共享 Execution/Lifecycle primitive；Conversation、Research Task 或未来 source 可创建，统一终态/operation 与来源归属合同 | 按 run_kind 保留合法 shape，Research 不强造 Conversation/Session |
...
**Admission 不变量：** 所有 surface 由同一个 PG authority 决定 admission，禁止 QQ 私建 mailbox/内存队列成为第二权威。

**首版推荐 policy（可替换）：** 沿用一 Conversation 一个 active Run，包括 queued/running/cancelling；两个 surface 同样返回领域 busy，QQ 映射为明确提示。当前 QQ 会 signal 后续消息，这个行为改变需产品验收。未来可换为 persistent queue、interrupt、append-to-current-run；相应修改 PG admission 事务/索引，不改变领域 owner。等待是否占 admission slot 由该策略决定，与 execution lease 无关。群聊非触发消息可做有限临时上下文；一旦选入模型，内容与来源纳入 snapshot。
...
| --- | --- | --- |
| Conversation / Message / Session | Conversation Domain / PG | Chat 的 source/context owner |
| Run | 共享 Execution/Lifecycle primitive / PG | 按 source_kind/source_ref 与 account ownership 关联来源；不要求聊天实体，不立即拆新 package |
| transcript / operation / lease | durable data plane / PG | Run 执行状态、CAS、fencing 与重试结果；不同于用户 Message |
| ModelInputSnapshot / ModelInputReview | Agent 模型输入能力 / PG | 不可变输入、冻结 AuthorizationPolicy 与逐快照授权决定；History 只引用，Trace 只投影 |
...
Run lifetime ≠ Execution lease lifetime。Run 可经历 running → waiting_review / waiting_approval / external_wait → running → completed；这是目标逻辑状态，实际 PG status/phase/wait_reason 映射在 W1 确定，当前未实现。lease 只覆盖模型、工具或需资源保护的活跃区间。任何 durable wait 均不长期占 Activity slot、workspace lock 或 execution lease；恢复重新 acquire 并传播新 fencing token。source-neutral 输入携带稳定 Run/来源/上下文引用，lease token 属于可更新的执行区间，不能永久封在初始 Run 输入中。W5 消费此基础合同，详见 03/06。

## 新扩展边界

模型输入按“冻结请求 → 权限投影 → 版本授权 → 执行原快照”衔接。准备覆盖 System/History/Memory/File/Plan/Tool、模型选择和有效参数；Invoke 不再组 prompt/选工具。等待期间输入或 provider fallback 改变实际请求，创建新 snapshot 并重新满足冻结 AuthorizationPolicy；需人工时才重审。首版 off/every_call，未来 first_call_only/phase_based/policy-based 不改变每次 Invoke 必须核验授权的不变量。见 06。
...
| web_domain | Conversation 核心提升中立领域；Run 逻辑上归共享 Execution/Lifecycle；其他 capability 保持 owner | 04 |
