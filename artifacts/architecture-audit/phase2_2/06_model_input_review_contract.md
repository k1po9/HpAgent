# ModelInputSnapshot / Model Review · 架构对象草案
...
**Target：** 同一条主模型调用链完成 prepare → freeze → authorize → invoke。这里审阅范围为主 Agent 的 decision、plan、replan、plan evaluation、step、forced final、synthesis；review enabled 推荐逐次调用审核，不能仅审第一轮后授权所有未知后续输入。辅助记忆检索改写可能发生在审阅前，须在产品说明/Trace 中可辨；不宣称“审批前没有任何模型调用”。Research 模型审阅是 FUTURE OPTION。
...
History 中仅保留 snapshot_ref/content_hash、review_id、operation/phase 等紧凑字段和确定性等待/信号控制；不放全文、工具参数大对象或文件内容。review 的权威授权记录在 PG，Workflow 控制推进在 Temporal；signal 是唤醒通知，不能凭其携带的 approved=true 直接授权调用。

## 能力边界

| 能力 | 允许职责 | 退出条件 |
| --- | --- | --- |
| ContextBootstrap | 从统一 PG 对话、Hindsight、File/Plan 准备上下文；辅助检索调用独立计量 | 可恢复 transcript/context refs，不产生主模型回复 |
...
| InvokeModel | 读取快照、核验状态/hash/当前授权/依赖/fencing，原样发送冻结请求并持久化结果 | 不重载历史、不补 system prompt、不重选工具、不改 effective 参数或偷偷 fallback |
...
flowchart TD
    ...
    S --> MODE{Frozen Run review policy}
    MODE -->|disabled| BY[Record explicit policy bypass]
    MODE -->|enabled| WAIT[Durable wait for review]
    WAIT -->|approve notification| CHECK[Load authoritative decision and verify]
    WAIT -->|reject / expire / cancel| STOP[Resolve Run without model invocation]
    CHECK -->|stale input / superseded| NEW[Invalidate old review and prepare new snapshot]
    NEW --> P
    CHECK -->|valid same snapshot| INV[Claim invocation and send frozen body]
    BY --> INV
    INV --> RES[Persist result / transcript CAS]
```

没有“reject 也汇入 Invoke”的分支。review disabled 也执行 Prepare/Freeze/Invoke 同一条链，明确记录冻结的 policy bypass，不能另保留旧 model_decision 调用旁路。建议 Run 仍占同 Conversation 的 active slot，用运行 phase/wait reason 投影等待状态；review 记录状态独立，最终 Run 状态如何映射 reject/expire 由生命周期合同选定，且必须终止等待、释放资源。
...
等待不占 Activity worker 槽、workspace 账户锁、数据库事务或网络连接。推荐释放长期占用的账户执行 lease，在恢复时重新 acquire，传播新 fencing token 给后续 Agent/Tool/生命周期 release；重验 transcript 与所有相关输入依赖。当前 AgentRunInput 固定携带 lease_token，因此这需要明确改造，不能直接沿用旧 token。无需物理 workspace 的模型执行使用冻结文件内容，真正 Tool 执行仍取得当前资源锁并另验风险审批。
...
ResourcePool 当前会 fallback；目标中换 endpoint/model/API format/有效参数必须新 snapshot，review enabled 时重新审批。同模型同语义请求的传输重试只能在原授权范围内进行；重新检索记忆、读取变更文件、工具 schema 刷新、plan/transcript 版本更新亦需旧批准失效。动态配置不允许在 Invoke 时覆盖准备阶段快照。
...
G12 必须用捕获实际请求 body 的模型假服务验证 hash 对应 payload，并覆盖所有主模型 phase；仅验证 UI 文本相同或给 snapshot 字段写单元测试不足以闭合合同。
