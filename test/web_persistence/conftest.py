...
def db(database_url: str, migration_database_url: str):
    with psycopg.connect(migration_database_url, autocommit=True) as connection:
        ...
        for table in ("artifact_outbox_events", "artifact_versions", "artifacts", "account_entitlements", "registration_invites", "identity_binding_challenges", "web_credentials", "outbox_events", "idempotency_commands", "workflow_executions", "messages", "runs", "sessions", "web_auth_sessions", "identity_bindings", "conversations", "accounts"):
