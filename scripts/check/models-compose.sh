...
# 测试实现复用 scripts/check/models.py。
set -euo pipefail
...
usage() {
  cat <<'EOF'
用法:
  ./scripts/check/models-compose.sh [包装器选项] [模型测试选项]
...
模型测试选项（透传给 scripts/check/models.py）:
  -n N                          每个模型调用次数（默认 5）
  -c, --categories CATEGORIES  模型类别，如 fast,chat（默认全部文本模型）
  --stream                     使用流式请求（仅可与 --direct 一起使用）

示例:
  ./scripts/check/models-compose.sh -n 10 -c fast,chat
  ./scripts/check/models-compose.sh --build -n 5
  ./scripts/check/models-compose.sh --direct --stream -c chat
...
docker compose -f "$COMPOSE_FILE" --profile agent run --rm --no-deps \
  ...
  --entrypoint python \
  --env PYTHONPATH=/app \
  --volume "$ROOT/scripts/check/models.py:/app/scripts/check/models.py:ro" \
  hpagent \
  /app/scripts/check/models.py \
  "${MODE_ARGS[@]}" \
  "${BENCHMARK_ARGS[@]}"
