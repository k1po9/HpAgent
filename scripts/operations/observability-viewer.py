...
class Observatory:
    ...
    def health(self) -> dict[str, Any]:
        ...
        return {
            "status": "ok", "sources": {
                "jsonl": {
                    "status": "ok" if self.logs.log_dir.exists() else "unavailable",
                    "log_dir": str(self.logs.log_dir.resolve()),
                    "exists": self.logs.log_dir.exists(),
                    "files": len(self.logs.states),
                    "events_cached": len(self.logs.events),
                    "parse_errors": self.logs.parse_errors,
                },
                "postgres": {
                    "status": "degraded" if self.postgres.last_error else ("ok" if self.postgres.database_url else "unavailable"),
                    "configured": bool(self.postgres.database_url),
...
async function poll(){if(document.hidden||pollInFlight)return;pollInFlight=true;try{const [e,h]=await Promise.all([get('/api/executions'),get('/api/health')]);executions=Array.isArray(e.executions)?e.executions:[];currentHealth=h;renderHealth(h);renderList();if(selected)applyDetail(await get('/api/execution/'+encodeURIComponent(selected)))}catch(e){console.error('viewer poll failed:',e);document.getElementById('health').innerHTML='<span class="badge failed">viewer degraded</span>'}finally{pollInFlight=false;schedulePoll()}}
...
def database_target(database_url: str | None) -> str:
    """Describe a database target without exposing credentials."""
    if not database_url:
        return "disabled"
    ...
    return f"{host}{port}/{database} (read-only)"
...
def main() -> None:
    ...
    jsonl_count = len(list(log_dir.glob("*.jsonl"))) if log_dir.exists() else 0
    print(f"HpAgent Execution Observatory: http://{args.host}:{args.port}")
    print(f"  JSONL root: {log_dir}")
    print(f"  JSONL exists: {log_dir.exists()}")
    print(f"  JSONL files: {jsonl_count}")
    print(f"  PostgreSQL: {database_target(args.database_url)}")
    try:
